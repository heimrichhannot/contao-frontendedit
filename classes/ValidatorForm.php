...
class ValidatorForm extends ReaderForm
{
    ...
    {
        $this->arrFields[FRONTENDEDIT_BUTTON_SUBMIT] = $this->generateField(
            FRONTENDEDIT_BUTTON_SUBMIT,
            [
                'inputType' => 'submit',
    ...
    public function runOnValidationError($arrInvalidFields)
    {
        ...
        StatusMessage::addError(
            sprintf(
                $GLOBALS['TL_LANG']['frontendedit']['validationFailed'],
                '<ul>' . implode(
                    '',
                    array_map(
                        function ($val) use ($arrDca)
                        {
