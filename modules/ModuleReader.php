...
class ModuleReader extends \Module
{
	...
	{
		if ($this->allowDelete && $this->addDeleteConditions && ($objItem = General::getModelInstance($this->formHybridDataContainer, $intId)) !== null) {
	...
	protected function addDefaultArchive()
	{
		if ($this->objModel->defaultArchive) {
			...
			$this->objModel->formHybridDefaultValues = array_merge(
				array(
					array(
						'field' => 'pid',
	...
	protected function generateFields($objItem)
	{
		...
		foreach ($this->arrEditable as $strName) {
			...
			// Convert timestamps
			if ($varValue != ''
				&& ($this->dca['fields'][$strName]['eval']['rgxp'] == 'date' || $this->dca['fields'][$strName]['eval']['rgxp'] == 'time'
